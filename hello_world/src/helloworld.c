...
int init_sd();
int read_kmer_files();
int init_axidma(XAxiDma* axi_dma, XAxiDma_Config *cfg_ptr, unsigned DMA_ID, char id);
...
int main() {
    ...
    init_platform();
    if (init_sd() != XST_SUCCESS) {
        xil_printf("No se pudo montar la uSD.");
        return 1;
    }
    if (read_kmer_files() != XST_SUCCESS) {
        xil_printf("Error al leer los archivos\r\n");
        return 1;
    }

    // Start PL timer con sus configs
    XTime_GetTime(&t_start0);

    XAxiDma_Config *CfgPtr0; 
    XAxiDma_Config *CfgPtr1; 

    init_axidma(&AxiDma0, CfgPtr0, DMA_0_ID, '0');
    init_axidma(&AxiDma1, CfgPtr1, DMA_1_ID, '1');
    ...
    // Start PL timer con transferencias + procesamiento de los datos
    ...
    // Stop PL timer
    XTime_GetTime(&t_end);
    u_time0 = (t_end-t_start0)*1000000/COUNTS_PER_SECOND; // configs
    u_time1 = (t_end-t_start1)*1000000/COUNTS_PER_SECOND; // solo trans+process
...
int write_histogram_file(unsigned* histogram, size_t n) {
    ...
    // 2. Escribir todos los bins y sus counts
    char s[10];
    char space = ' ', endl = '\n', colon = ':';
    ...
    xil_printf("Archivo del histograma escrito.\r\n");
    f_close(&fil2);
...
int read_kmer_files() {
    ...
    for (int i = 0; i < NUM_KMERS_1; i++) {
        ...
        f_read(&fil0, &TxBuffer0[i * KMER_TOTAL_BYTES], KMER_TOTAL_BYTES, &bytesRead);
        TxBuffer0[(i * KMER_TOTAL_BYTES) + 31] = 0x00; // Padding
    }
    for (int i = 0; i < NUM_KMERS_2; i++) {
        // Leer 31 bytes del archivo 1
        f_read(&fil1, &TxBuffer1[i * KMER_TOTAL_BYTES], KMER_TOTAL_BYTES, &bytesRead);
...
int init_axidma(XAxiDma* axi_dma, XAxiDma_Config *cfg_ptr, unsigned DMA_ID, char id) {
	...
	cfg_ptr = XAxiDma_LookupConfig(DMA_ID);
	if (!cfg_ptr) {
		xil_printf("No config found for DMA%c\r\n", id);
		return XST_FAILURE;
	}

	status = XAxiDma_CfgInitialize(axi_dma, cfg_ptr);
	if (status != XST_SUCCESS) {
		xil_printf("DMA%c initialization failed %d\r\n", id, status);
		return XST_FAILURE;
	}
